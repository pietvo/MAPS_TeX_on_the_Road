...
\begin{figure}[hb]
  \centering
  \noindent\hbox to 0.9\columnwidth{\includegraphics[width=0.45\columnwidth]{enumerate}\hss}\\~
...
iPad keyboard takes so much space that hardly any source code is visible. See
...
So I had to give it a \texttt{.tex} file extension to make it (and myself) happy. I
tried two ways
\begin{itemize}
...
section `Latexmk' on page~\pageref{sec:latexmk}.
...
the file names, to keep a kind of history. But this soon becomes unwieldy.
...
sets of changes must then be \emph{merged}. This is the simple explanation.
...
Working Copy's editor has syntax highlighting for  more than 50 different languages. It can show nice graphical representations of your branches and your commit history (see fig.~\ref{fig:WC-history}). Besides the \texttt{merge} functionality it also has the \texttt{rebase} functionality, which is an alternative for merge. For cooperating in large projects this functionality is sometimes necessary.
...
As I have mentioned above, all these apps have the facility that you can open their files in an external editor. Figure~\ref{fig:open-from-Git2Go} shows how to open files from Git2Go in Textastic. This is done inside Textastic with the ``Open\ldots'' button, then selecting ``Git2Go''. It is then possible to choose ``Open'' down in the pop-up, which will open the whole directory in Textastic, or select one filename, which will open that file.
