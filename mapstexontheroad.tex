...
\begin{keywords}\LaTeX, travelling, iPad, Overleaf, git, distributed
...
As I have mentioned above, all these apps have the facility that you can open their files in an external editor. Figure~\ref{fig:open-from-Git2Go} shows how to open files from Git2Go in Textastic. This is done inside Textastic with the ``Open\ldots'' button, then selecting ``Git2Go''. It is the possible to choose ``Open'' down in the pop-up, which will open the whole directory in Textastic, or to select one filename, which will open that file.
